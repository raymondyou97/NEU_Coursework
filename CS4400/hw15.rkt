...
;; helper for executing receivers, wraps the producer value in a RktV if
;; needed, and calls the receiver with this value (if it is a valid
;; receiver value).  Note that the value is actually a `producer' thunk,
;; because it might involve some side-effect (like reading) that we want
;; to avoid if the receiver is invalid.
(define (execute-receiver receiver producer)
  (cases receiver
    [(FunV names body env)
     (let [(val (producer))]
           (execute-val
            (eval body (extend names (list (wrap-in-val val)) env))))]
...
(define (execute-unref input receiver)
  (cases input
    [(RktV ref)
     (if (ref? ref)
         (execute-receiver receiver (lambda () (unref ref)))
         (error 'execute-unref "invalid ref"))]
    [else (error 'execute-unref "expecting a Ref")]))

(: execute-set-ref : VAL VAL -> Void)
;; execute a `set-ref' description
(define (execute-set-ref input val)
  (cases input
    [(RktV ref)
     (if (ref? ref)
         (set-ref! ref val)
         (error 'execute-setref "invalid ref"))]
    [else (error 'execute-setref "expecting a Ref")]))
...
(test (run-io "{unref + 1}") =error> "expecting a Ref")
(test (run-io "{unref {print ''} 1}") =error> "invalid ref")

(test (run-io "{set-ref! + 1}") =error> "expecting a Ref")
(test (run-io "{set-ref! {print ''} 1}") =error> "invalid ref")
