...
(test (not (div5 (number->string 123456 2))))
...
;; A macro that defines a PDA (pushdown automata) language
(define-syntax pushdown
  (syntax-rules (: ->)
    [(pushdown init-state end-state
        ; PDA reads incoming input and top of stack to determine next state
        [state : ((input-sym ...) (stack-top ...)
          -> new-state (push-down ...)) ...]
        ...)
     (lambda (string)
