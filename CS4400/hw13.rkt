...
(define (compile-body exprs)
  (let ([compiled-exprs (map compile exprs)])
    (lambda (env)
      (let ([results (map (caller env) compiled-exprs)])
        (: last : (Listof VAL) -> VAL)
        (define (last lst)
          (if (null? (rest lst))
              (first lst)
...
(define (compile expr)
  (unless (unbox compiler-enabled?)
   (error 'compile "compiler disabled ~s" expr))
  (cases expr
    ...
    [(Set name new)
     (let ([compiled-new (compile new)])
       (lambda ([env : ENV])
         (set-box! (lookup name env) ((caller env) compiled-new))
         the-bogus-value))]
    ...
    [(BindRec names exprs bound-body)
     (let ([compiled-exprs (map compile exprs)]
           [compiled-body (compile-body bound-body)])
       (lambda ([env : ENV]) (compiled-body
                              (extend-rec names compiled-exprs env))))]
    [(Fun names bound-body)
     (let ([compiled-bound-body (compile-body bound-body)])
       (lambda ([env : ENV]) (FunV names compiled-bound-body env #f)))]
    [(RFun names bound-body)
     (let ([compiled-bound-body (compile-body bound-body)])
       (lambda ([env : ENV]) (FunV names compiled-bound-body env #t)))]
    [(Call fun-expr arg-exprs)
     (let ([compiled-arg-exprs (map compile arg-exprs)]
           [compiled-fun-expr (compile fun-expr)]
           [compiled-boxes (compile-get-boxes arg-exprs)])
       (lambda ([env : ENV])
         (let ([fval ((caller env) compiled-fun-expr)]
               ;; delay evaluating the arguments
               [arg-vals (lambda () (map (caller env) compiled-arg-exprs))])
           (cases fval
    ...
    [(If cond-expr then-expr else-expr)
     (let ([compiled-cond-expr (compile cond-expr)]
       ...
       (lambda ([env : ENV])
         (if (cases ((caller env) compiled-cond-expr)
                         [(RktV v) v] ; Racket value => use as boolean
                         [else #t])   ; other values are always true
                       ((caller env) compiled-then-expr)
                       ((caller env) compiled-else-expr))))]))
...
(test (compile (parse "asdfasdf")) =error> "compiler disabled")
(test (compile-get-boxes (list (parse "{asdfa}"))) =error> "compiler disabled")
