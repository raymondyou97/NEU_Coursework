...
(define/rec append
  (lambda (list1 list2)
    (if (null? list1)
        list2
        ;;recursively build new list starting with first list, then second list
...
(define/rec filter
  (lambda (func list)
    (if (null? list)
        null
        (if (func (car list))
            (cons (car list) (filter func (cdr list))) ;; add first to list
            (filter func (cdr list)))))) ;; else recurse on rest
;; tests
(define =3 (= 3))
(define =4 (= 4))
(test (->listof ->nat (filter =3 null)) => '())
(test (->listof ->nat (filter =3 l123)) => '(3))
(test (->listof ->nat (filter =4 l123)) => '())
...
;; determines whether it's safe to put a queen at column col when it's
...
(test (->bool (safe? 1 2 l123)) => '#f)
(test (->bool (safe? 1 3 l123)) => '#f)
...
(define/rec range
  (lambda (from to)
    (if (= from to)
        null ;; don't include
        (cons from (range (add1 from) to))))) ;; recursively build list
;; tests
...
(define queens
  (lambda (size)
    (with [solutions (configurations size (range 0 size))]
          (if (null? solutions)
              null
              (car solutions))))) ;; only return the first solution
;; tests
...
;; queens : Nat -> (Listof Nat)
;; returns list of all solutions for a given board size
(define queens-all
  (lambda (size)
...
(test (->listof (->listof ->nat) (queens-all 4)) => '((2 0 3 1) (1 3 0 2)))
(test (->listof (->listof ->nat) (queens-all 5))
      => '((3 1 4 2 0) (2 4 1 3 0) (4 2 0 3 1) (3 0 2 4 1) (4 1 3 0 2)
                       (0 3 1 4 2) (1 4 2 0 3) (0 2 4 1 3) (2 0 3 1 4)
                       (1 3 0 2 4)))

(define hours-spent (+ 2 2))
