...
A stack is quite useful as it allows us you add elements to a list and only get elements from the top (aka the most recently added element). This is a LIFO (last in, first out) retrieval. This is especially useful if we want a data structure where you are only able to pop the most recently added element, so for example linked lists.
